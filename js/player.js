...
function switchIntroTab(tab) {
    document.getElementById('publicTab').classList.toggle('active', tab === 'public');
    document.getElementById('hiddenTab').classList.toggle('active', tab === 'hidden');
    ...
    var chars = [
        {
            ...
            truth: '他其实是你童年时的邻居哥哥，当年惨案发生时，他因为懦弱没有救你，一直在深深的自责中寻找复活你的方法。'
        },
        {
            ...
            truth: '他曾是你最讨厌的校霸，但在你被继母虐待时，只有他偷偷给你送过药。棒球棍是他为了保护你而准备的武器。'
        },
        {
            ...
            truth: '他并不是人类，而是你绝望中幻想出的守护灵。他的存在就是为了吸收你的痛苦，一旦你离开洋楼，他就会消散。'
        }
    ];

    var contentDiv = document.getElementById('characterIntroContent');

    if (tab === 'public') {
        contentDiv.innerHTML = chars.map(function(data) {
            var traitsHtml = data.traits.map(function(t) { return '<li>' + t + '</li>'; }).join('');
            return '<div class="char-intro-card">' +
                '<div class="char-intro-header"><div class="char-intro-name">' + data.name + '</div><div class="char-intro-title">' + data.title + '</div></div>' +
                '<div class="char-intro-section"><div class="char-intro-label">📝 角色设定</div><div class="char-intro-text">' + data.desc + '</div></div>' +
                '<div class="char-intro-section"><div class="char-intro-label">✨ 性格特质</div><ul class="char-intro-list">' + traitsHtml + '</ul></div>' +
                '<div class="char-intro-section"><div class="char-intro-label">📜 背景剧情</div><div class="char-intro-text">' + data.plot + '</div></div>' +
                '</div>';
        }).join('');
    } else {
        contentDiv.innerHTML = chars.map(function(data) {
            var charAff = affectionData && affectionData.characters && affectionData.characters.find(function(c) { return c.name === data.name; });
            var unlocked = charAff ? charAff.value >= 70 : false;
            return '<div class="char-intro-card">' +
                '<div class="char-intro-header"><div class="char-intro-name">' + data.name + '</div></div>' +
                (unlocked
                    ? '<div class="char-intro-section"><div class="char-intro-label">🔓 核心真相</div><div class="char-intro-text" style="color:#ff6b6b;font-weight:bold;">【真相碎片】' + data.truth + '</div></div>'
                    : '<div class="char-intro-section locked-hint"><div class="lock-icon">🔒</div><div>好感度达到70解锁核心真相</div></div>'
                ) +
                '</div>';
        }).join('');
    }
